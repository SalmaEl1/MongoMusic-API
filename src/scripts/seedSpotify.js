...
const getArtistAlbums = async (artistId, accessToken, limit = 20) => {
  const data = await spotifyGet(`/artists/${artistId}/albums?include_groups=album,single&limit=${limit}`, accessToken);
...
const seedFromSpotify = async () => {
  ...
  for (const artistName of artistNames) {
    ...
    artistsBySpotifyId.set(spotifyArtist.id, spotifyArtistFull);
  }
  ...
  for (const album of allAlbums) {
    ...
    for (const track of tracks) {
      ...
      const isSoloTrackByMainArtist = trackArtistNames.length === 1 && trackArtistNames[0] === normalizedMainArtistName;
